...
```{r}
salary5 = mutate(salary4, YrsEmployed = floor(2019-HireYear))
str(salary5)
```
...
```{r}
salary.final = salary5 %>% select(Name, Department, Job, Exempt, Salary, YrsEmployed) %>% arrange(Department) %>% arrange(YrsEmployed) %>% select(YrsEmployed, Salary, everything())
head(salary.final)
```
...
Code *(1 Point)*:
```{r}
ling_sal <- salary.final[which(salary.final$Department == 'Linguistics'),]
```

Answer *(1 Point)*:
```{r}
print(paste('The average salary of employees in the Linguistics Department is',mean(ling_sal$Salary), 'dollars.'))
```
...
Code *(2 Points)*:
```{r}
fam_med_6exempt <- salary.final[which((salary.final$Department == 'Family Medicine') & (salary.final$YrsEmployed <= 6) & (salary.final$Exempt == 'Exempt From Personnel Act') )]
```

Answer *(2 Points)*: (Place Answer Here Using Complete Sentences)
```{r}
print(paste(nrow(fam_med_6exempt), 'employees in Family Medicine have worked for 6 years or less and are exempt.'))
```
...
Code *(2 Points)*:
```{r}
med_sal_PPD = salary.final %>% filter((Department == 'Public Policy') & (Job == 'Professor' | Job == 'Associate Professor' | Job == 'Assistant Professor'))

```

Answer *(2 Points)*: (Place Answer Here Using Complete Sentences)
```{r}
print(paste(median(med_sal_PPD$Salary), 'dollars is the median salary of associate, assistant, and full professors.'))
```
...
```{r}
deptyear_summary = salary.final %>% group_by(Department, YrsEmployed) %>% summarize(n = n(), mean = mean(Salary), sd = sd(Salary), prop70 = 0) %>% ungroup(deptyear_summary)
...
Code *(2 Points)*:
```{r}
low <- deptyear_summary
low <- deptyear_summary %>% filter(YrsEmployed == 3)
low <- low[order(low$mean),]
low <- low[1:3,]
head(low)
```

Answer *(2 Points)*: The NaN values are for datasets where there is only one value. Since there is only one value, it cannot calculate the standard deviation.
...
```{r}
ggplot(deptyear_summary, mapping = aes(x = YrsEmployed, y = mean)) + geom_point() + geom_smooth(method="lm")
```

Answer *(2 Points)*: The plot's linear relationship shows a weak positive association between years worked and average salary. This makes sense because as one continues to work, they receive raises.
...
```{r}
three_dept <- deptyear_summary %>% filter(Department == 'Mathematics' | Department == 'Computer Science' | Department == 'Chemistry')

ggplot(three_dept, mapping = aes(x = YrsEmployed, y = mean, color = Department)) + geom_point() + geom_smooth(method="lm", se=F)
```

Answer *(3 Points)*: I chose the Computer Science, Chemistry, and Mathematics departments to study because they are so similar in what they teach. Interestingly, Computer Science begins as the lowest paying department but ends as the highest by far. Inversely, Mathematics does the complete opposite. Computer Science offers the highest salary increases follwed by Chemistry and then Mathematics. 

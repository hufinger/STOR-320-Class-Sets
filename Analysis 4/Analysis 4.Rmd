...
**Assigned:** Friday, April 5, 2018

**Due:** Friday, April 19, 2018 by 5:00 PM
...
DATA2 = DATA %>% group_by(L) %>% crossv_kfold(10) 
...
for(row in 1:nrow(OUT.RMSE)){
  for(col in 1:ncol(OUT.RMSE)){
    DATA3 = DATA2 %>% mutate(tr.model=map(train, train.model.func,row, col)) 
    DATA4 = DATA3 %>% mutate(predict=map2(test,tr.model,~augment(.y,newdata=.x))) %>% select(predict) %>% unnest()
    OUT.RMSE[row, col] = RMSE.func(actual = DATA4$W, predict = DATA4$.fitted);
  }
}
...
Code and Output:
```{r,eval=T}
BEST5.RMSE = OUT.RMSE2 %>% arrange(RMSE)
BEST5.RMSE = BEST5.RMSE[1:5,]
...
Code and Output:
```{r,eval=T}
BEST5.DATA2 = BEST5.DATA %>% gather('First', 'Second', 'Third', 'Fourth', 'Fifth', key = "Model", value = "Predict", factor_key = T)
...
Answer *(2 Points)*: The difference in the top models is minimal at best. They all follow the curve of the data in a similar fashion.
...
Answer: 3 is clearly the I value to use based on the graph. 3 has the lowest RMSE of all the values. For J, 6 appears to have the lowest RMSE value. 7 is close but it looks like 6 is the lowest. 
...
Code and Output:
```{r,eval=T}
ggplot(DATA)+geom_point(aes(x = W, y = predict(train.model.func(DATA,3,6))), alpha=0.15,stroke=0) + xlab("Actual Max Water Temperature")+ylab("Fitted Max Water Temperature") + geom_abline(intercept = 0,slope = 1, color = "red")
```
...
Code and Output:
```{r,eval=T}
ggplot(DATA) + geom_hline(yintercept = 0, color = "red")+geom_point(aes(x = D, y = residuals(train.model.func(DATA,3,6))), alpha=0.15,stroke=0) + xlab("Day")+ylab("Residual")
```